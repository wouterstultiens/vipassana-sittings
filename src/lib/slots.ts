...
// works out what a row shows: its tags, and whether it has ended.
import type { Sitting } from "@/lib/expand";
import { fmtDuration, languageFlag, languageLabel, type LanguageFlag } from "@/lib/labels";
...
export type LanguageTag = { flag: LanguageFlag | null; codes: string[]; label: string };

/** What varies between slots: languages without English, and a length other than one hour. */
export function slotTags(slot: Slot): { languages: LanguageTag[]; length: string | null } {
  ...
  for (const code of codes) {
    ...
    else byFlag.set(flag ?? code, { flag, codes: [code], label: "" });
  }
  const languages = [...byFlag.values()].map((t) => ({ ...t, label: t.codes.map(languageLabel).join(", ") }));
  return { languages, length: slot.durationMinutes === 60 ? null : fmtDuration(slot.durationMinutes) };
}
