...
// listing pass, and does this sitting pass.
...
export type Filters = {
  weekdays: number[]; // 0 = Sunday, in the visitor's zone
...
/** The filters that a listing can answer on its own, used by the card list too. */
export function listingMatches(listing: Listing, f: Filters): boolean {
  ...
  if (f.platforms.length && !f.platforms.includes(listing.platform)) return false;
